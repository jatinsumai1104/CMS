    ...
    <div class="container">
      ...
      <div class="row">
		...
		if(isset($_GET['post_id'])){
			...
			$result = $post->readPostByPostId($post_id);
		?>
		
          <!-- Title -->
          <h1 class="mt-4"><?php echo $result['post_title']?></h1>

          <!-- Author -->
          <p class="lead">
            by
            <a href="<?php echo BASEURL."author/{$result['post_author_id']}";?>"><?php echo $post->getAuthorName($result['post_author_id']); ?></a>
          </p>

          <hr>

          <!-- Date/Time -->
          <p><span>Posted on <?php echo $result['post_date']; ?></span>
          	<span>
          	<?php 
				$tag = explode(",", $result['post_tags']);
				$URL = BASEURL;
...
TAG;
          ...
          <!-- Preview Image -->
          <img class="img-fluid rounded" src="<?php echo POSTIMAGES."".$result['post_id']."".$result['post_image_extension'];?>" alt="<?php echo $result['post_title'];?>">

          <hr>

          <!-- Post Content -->
          <p class="lead"><?php echo $result['post_body'];?></p>
